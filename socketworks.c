...
//              LOG("select returned %d",rv);
	...
	while (++i < max_sock)
	  if (pf[i].revents)
		...
		rlen = read (ss->sock, &ss->buf[ss->rlen], ss->lbuf - ss->rlen);	// not an server socket

	      ss->rlen += rlen;
	      ss->buf[ss->rlen] = 0;	//force 0 at the end of the string
      ...
      if (rv == 0 || c_time - lt > 200)	// checking every 60seconds for idle connections - or if select times out
	{
	  lt = c_time;
	  i = -1;
...
get_mac (char *mac)
{
  ...
  if (opts.mac[0])
    {
      strncpy (mac, opts.mac, 13);	// simulate mac address
      return 0;
    }
