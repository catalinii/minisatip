...
SPMT *get_pmt(int id)
{
	if(id <0 || id >= MAX_KEYS || !pmt[id] || !pmt[id]->enabled)
		return NULL;
	return pmt[id];
}

int register_algo(dvbapi_op *o)
{
	int i;
...
void update_pid_key(adapter *ad)
{
	...
	if (ad->pid_to_pmt && ad->pid_to_key[0]) // cache pid_to_key
	{
...
int pmt_valid_cw(int aid, int pmt_id)
{
	SPMT *p = get_pmt(aid, pmt_id);
	if(p->active_cw[p->parity])
		return 1;
	return 0;
}
...
int pmt_decrypt_stream(adapter *ad)
{
	...
	for (i = 0; i < rlen; i += 188)
	{
		...
		if (b[3] & 0x80)
		{
			if (dvbapi_is_enabled && ad->pid_to_pmt)
				pmt_id = ad->pid_to_pmt[pid];
			else
				pmt_id = -1;
			if ((pmt_id == -1) || )
			{
