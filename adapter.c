...
close_adapter (int na)
{
  ...
  a[na].enabled = 0;
  close_streams_for_adapter (na, -1);	//close all streams attached to this adapter
...
get_free_adapter (int freq, int pol, int msys, int src)
{
  ...
  for (i = 0; i < MAX_ADAPTERS; i++)
    if (a[i].enabled && a[i].sid_cnt == 0 && a[i].tp.sys == msys)	//first free1 adapter that has the same msys
      return i;
  if (msys == SYS_DVBS)
    msys = SYS_DVBS2;
  if (msys == SYS_DVBT)
    msys = SYS_DVBT2;
  for (i = 0; i < MAX_ADAPTERS; i++)
    if (a[i].enabled && a[i].sid_cnt == 0 && a[i].tp.sys == msys)	//first free1 adapter that supports also msys (for example: DVBS2 will match DVBS streams)
      return i;
  for (i = 0; i < MAX_ADAPTERS; i++)
...
//      dump_pids(aid);
  mark_pids_deleted (aid, sid, NULL);	// delete the attached PIDs as well
//      dump_pids(aid);
  update_pids (aid);
  dump_pids (aid);
  if (a[aid].sid_cnt == 0)
...
void
mark_pids_deleted (int aid, int sid, char *pids)	//pids==NULL -> delete all pids
{
  int i,
    j,
    la,
  ...
  for (i = 0; i < MAX_PIDS; i++)
      ...
      if (pids == NULL)
	{
	  ...
	  for (j = 0; j < MAX_STREAMS_PER_PID; j++)
	    if (sid == -1)
	      ad->pids[i].sid[j] = -1;	// delete all pids if sid = -1
	    else if (ad->pids[i].sid[j] == sid)
	      ad->pids[i].sid[j] = -1;	// delete all pids where .sid == sid
	  if (sid != -1)
	    {
	      int cnt = 0;
      ...
      else
	{
	  ...
	  for (j = 0; j < MAX_STREAMS_PER_PID - 1; j++)	//make sure that -1 will be after all the pids
...
//              LOG("processing pid %d",p[j]);
      for (i = 0; i < MAX_PIDS; i++)	// check if the pid already exists, if yes add the sid
	if (ad->pids[i].flags > 0 && ad->pids[i].pid == p[j])
      ...
      if (!found)
	for (i = 0; i < MAX_PIDS; i++)
	  if (ad->pids[i].flags <= 0)	// if no mark the pid for add
	    {
	      ad->pids[i].flags = 2;
	      ad->pids[i].pid = p[j];
	      ad->pids[i].sid[0] = sid;
...
set_adapter_parameters (int aid, int sid, transponder * tp)
{
  ...
  LOG ("setting DVB for adapter %d - master_sid %d sid %d old f:%d", aid,
  ...
  if (a[aid].sid_cnt == 1 && tp->pids)	// just 1 stream per adapter and pids= specified
    {
      mark_pids_deleted (aid, -1, NULL);
      update_pids (aid);
    }
  ...
  if (a[aid].tp.apids || a[aid].tp.pids)
    {
      if (mark_pids_add
...
describe_adapter (int aid)
{
  ...
  if (x)
    dad[strlen (dad) - 1] = 0;	// cut the last comma
  else
    dad[strlen (dad)] = '0';
  return dad;
}
